...
class AsyncCoreDataTestStack {
    ...
    func clearData() async throws {
        ...
        try viewContext.performAndWait { // Use performAndWait for synchronous execution on context
...
struct BooksImporterTests {
    ...
    func testParseMixedEntries() {
        ...
        let books = BooksImporter.parseBooks(from: output)
        #expect(books.count == 2) // "Valid Book" and "Another Valid" should be parsed
        #expect(books[0].title == "Valid Book")
        #expect(books[1].title == "Another Valid")
    }
